...
pub(crate) fn generate_dataset(config: &Config) -> Result<(), Box<dyn Error>> {
    let projects = config
        ...
        .map(|vpp_path| {
            ...
            let source_sha256 = hex_sha256(&fs::read(vpp_path)?);
            Ok::<_, Box<dyn Error>>((vpp_path.clone(), project, source_sha256))
        })
    ...
    for (source_index, (vpp_path, project, source_sha256)) in projects.iter().enumerate() {
        ...
        for (block_index, block) in selected_blocks {
            ...
            for variant in variants {
                ...
                validate_sbv2(&sbv2)?;
                let sample_id = if projects.len() == 1 {
                    format!("b{block_index:03}_v{:03}", variant.index)
                } else {
                    format!("s{source_index:03}_b{block_index:03}_v{:03}", variant.index)
                };
                ...
                let julius_phone_rel_path =
                    format!("julius/{speed_directory}/phones/{sample_id}.txt");
                ...
                let mut julius_record = if ffmpeg.available {
                    let julius_audio_rel_path =
                        format!("julius/{speed_directory}/wav/{sample_id}.wav");
                    let julius_lab_rel_path =
                        format!("julius/{speed_directory}/wav/{sample_id}.lab");
                    let julius_audio_path = julius_output_root
                        .join(&speed_directory)
                        .join("wav")
                        .join(format!("{sample_id}.wav"));
                    let julius_lab_path = julius_audio_path.with_extension("lab");
                    let conversion = convert_wav(&ffmpeg, &audio_path, &julius_audio_path)
                        .and_then(|_| {
    ...
    let speed_groups = speed_writers
        ...
        .collect::<Vec<_>>();
    let primary_source = source_records.first().cloned();
    let manifest = json!({
        "schema_version": "vpp-sbv2-compatible-2",
        ...
        "counts": {
            "vpp_files": projects.len(),
            "blocks_total": projects
                .iter()
                .map(|(_, project, _)| project.project.blocks.len())
                .sum::<usize>(),
...
pub(crate) fn print_plan(vpp_path: &std::path::Path, project: &ProjectFile, config: &Config) {
