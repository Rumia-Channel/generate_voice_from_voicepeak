...
foreach $f (@files) {
    print $f . ".wav\n";
    @words=();
    if ($disable_silence_at_ends == 0){ push(@words, "silB"); }
    ...
    if ($disable_silence_at_ends == 0){ push(@words, "silE"); }
    close(TRANS);
    $num = $#words;
    ...
    close(DICT);
    if ((! -r "$f.dfa") || (! -f "$f.dfa")) { die "Error: failed to make \"$f.dfa\"\n"; }
    if ((! -r "$f.dict") || (! -f "$f.dict")) { die "Error: failed to make \"$f.dict\"\n"; }

    $command = "echo $f.wav | $juliusbin -h $hmmdefs -dfa $f.dfa -v $f.dict";
    $command .= " -palign";
    if ($hlist ne "") { $command .= " -hlist $hlist"; }
    $command .= " $OPTARGS";
    ...
    while(<LOG>) {
        chomp;
        if (/begin forced alignment/) { $sw = 1; }
        if ($sw == 1 && /^\[/) {
            if ($num > 0) {
                if(/\[(w_\d+)\]/){
                    $c_str = $1;
